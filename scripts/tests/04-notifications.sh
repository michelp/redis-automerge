...
redis-cli -h "$HOST" del notif_test2 > /dev/null
load_notif_test2() {
    redis-cli -h "$HOST" --raw -x am.load notif_test2 < /tmp/notif_load.bin
}
test_notification "notif_test2" "am.load" load_notif_test2
...
json='{"name":"test"}'
fromjson_notif() {
    printf '%s' "$json" | redis-cli -h "$HOST" -x am.fromjson notif_fromjson
}
test_notification "notif_fromjson" "am.fromjson" fromjson_notif
