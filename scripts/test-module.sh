...
# Test AM.TOJSON command
echo "66. Testing AM.TOJSON with empty document..."
...
# Verify nested structure with jq
...
# Verify array structure with jq
...
# Verify mixed types in array with jq
...
compact=$(redis-cli -h "$HOST" --raw am.tojson json_test6 false)
# Get pretty JSON
...
# Compact should be single line, pretty should be multiple lines
if [ "$compact_lines" -gt 1 ]; then
    echo "   ✗ Compact JSON has too many lines: $compact_lines"
    exit 1
fi
if [ "$pretty_lines" -le 1 ]; then
    echo "   ✗ Pretty JSON does not have enough lines: $pretty_lines"
    exit 1
fi
echo "   ✓ AM.TOJSON pretty formatting works"

echo "72. Testing AM.TOJSON with complex structure..."
...
# Verify complex nested structure with jq
...
redis-cli -h "$HOST" del json_test8
redis-cli -h "$HOST" --raw -x am.load json_test8 < /tmp/json_test8.bin
# Get JSON after reload
json_after=$(redis-cli -h "$HOST" --raw am.tojson json_test8)
# Compare values using jq to handle potential ordering differences
...
# Test AM.FROMJSON command
echo "74. Testing AM.FROMJSON with simple types..."
...
# Compare using jq to handle potential ordering differences
...
redis-cli -h "$HOST" del fromjson_test7
redis-cli -h "$HOST" --raw -x am.load fromjson_test7 < /tmp/fromjson_test7.bin
...
echo "81. Testing AM.FROMJSON with empty object..."
...
redis-cli -h "$HOST" del notif_fromjson
json='{"test":"value"}'
...
echo "90. Testing basic AOF persistence after restart..."
...
echo "91. Testing multiple documents persist after restart..."
...
echo "92. Testing nested paths persist after restart..."
...
echo "93. Testing persistence after AOF rewrite..."
...
echo "94. Testing comprehensive persistence scenario..."
